...
const AllAdmission = () => {
  const { data, isLoading } = useGetAllAdmissionQuery(undefined);
  const [deleteAdmission] = useDeleteAAdmissionMutation();
  ...
  const handleEditClick = (admissionId: string) => {
  ...
  const handleDeleteClick = async (admissionId: string) => {
    const toastId = toast.loading("Deleting admission...");
    try {
      const res = (await deleteAdmission(admissionId)) as any;
      if (res?.error?.data) {
        toast.error(res.error.data.message, { id: toastId });
      } else {
        toast.success("Deleted admission successfully", { id: toastId });
      }
    } catch (err) {
      console.error(err);
  ...
  const filteredAdmissions = data?.data.filter((admission: any) =>
