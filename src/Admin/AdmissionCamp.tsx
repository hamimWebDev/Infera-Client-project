...
const AllAdmissionCamp = () => {
  const { data, isLoading } = useGetAllAdmissionCampQuery(undefined);
  ...
  const filteredAdmissionCamp = data?.data.filter((admissionCamp: any) =>
    admissionCamp?.upazila?.toLowerCase().includes(searchQuery.toLowerCase())
  );
...
export default AllAdmissionCamp;
